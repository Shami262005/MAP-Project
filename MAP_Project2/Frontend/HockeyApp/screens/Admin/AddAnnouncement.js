...
export default function AddAnnouncementScreen({ navigation }) {
  ...
  const handleCreate = async () => {
    if (!title.trim() || !description.trim() || !imageUri) {
      return Alert.alert(
        'Missing fields',
        'Please provide title, description, and an image.'
      );
    }
    ...
    try {
      ...
      const url   = API.defaults.baseURL + '/announcements';

      // Send via fetch for reliable multipart handling
